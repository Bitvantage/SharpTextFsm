...
{
    internal class ThrowOnConversionFailureFalse : ITemplate
    {
        ...
        public void Test()
        {
            var template = Template.FromType<ThrowOnConversionFailureFalse>();
            var data = """
                P100X
                F200Y
                """;

            var results = template.Parse<ThrowOnConversionFailureFalse>(data).ToList();

            Assert.That(results.Count, Is.EqualTo(1));
            Assert.That(results[0].ValueProperty, Is.EqualTo(null));
            Assert.That(results[0].ValueField, Is.EqualTo(0));
        }
    }
