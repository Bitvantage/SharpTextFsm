...
namespace Test.Generic.Mappings
{
    internal class ExplicitMatch : ITemplate
    {
        ...
        public void Test()
        {
            var template = Template.FromType<ExplicitMatch>();
            var data = """
                P100
                F200
                """;

            var results = template.Parse<ExplicitMatch>(data).ToList();

            Assert.That(results.Count, Is.EqualTo(1));
            Assert.That(results[0].ValuePropertyX, Is.EqualTo(100));
            Assert.That(results[0].ValueFieldX, Is.EqualTo(200));
        }
    }
